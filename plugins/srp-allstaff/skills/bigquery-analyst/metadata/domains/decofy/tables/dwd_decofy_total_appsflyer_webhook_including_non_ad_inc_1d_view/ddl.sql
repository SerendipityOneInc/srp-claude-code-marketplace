CREATE VIEW `srpproduct-dc37e.favie_dw.dwd_decofy_total_appsflyer_webhook_including_non_ad_inc_1d_view`
AS SELECT  dt
	   ,'Decofy' as app_name
	   ,case when lower(media_source) like '%facebook%' or lower(media_source)  like '%360security_int%'  or lower(media_source)  like '%meta%' then 'Meta Ads'
			...
			when lower(media_source) like '%apple%' then 'Apple Search Ads' else media_source end as source
	   ,channel
...
(
	...
	FROM `favie_dw.dwd_decofy_appsflyer_webhook_inc_1d_view`
	...
	(
		...
		SELECT  Date(install_date) as dt
			   ...
			   ,install_date as event_time
		FROM `favie_dw.dwd_decofy_skan_appsflyer_webhook_inc_1d_view`
		where coalesce(af_attribution_flag,false)  !=true and event_name in  ('install') 
