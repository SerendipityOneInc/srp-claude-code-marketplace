# dwd_gem_product_image_index_update_inc_1d_function

**函数全名**: `srpproduct-dc37e.favie_dw.dwd_gem_product_image_index_update_inc_1d_function`
...
with source_data as (
    ...
    today_hour_increment as (
        ...
        from(
            select 
                ...
                product_update_time,
                record_time,
                row_number() over(partition by f_sku_id order by product_update_time desc) as rn
            from favie_dw.dwd_gem_product_image_index_inc_1h
    ...
    product_with_image as (
        SELECT
            ...
            COALESCE(T4.local_price, CAST(JSON_VALUE(T1.price,"$.value") AS INT64)) AS local_price, 
            ...
            COALESCE(T4.base_price, CAST(JSON_VALUE(T1.rrp,"$.value") AS INT64)) AS base_price, 
            ...
            COALESCE(
                ...
                CASE
                    ...
                    ELSE 0 
                END
            ) AS is_used, -- 从 `extended_info` 直接解析

            COALESCE(T4.inventory, T1.inventory) AS inventory,
            COALESCE(T4.f_status, T1.f_status) AS f_status,
    ...
    select 
        ...
        T1.f_status,
        T1.created_time,
        --必须提前通过join merge好图片数据，在merge table中不能使用udf进行merge
        favie_dw.emb_images_merge_udf(full_index_data.emb_images, T1.emb_images) AS emb_images,
        T1.product_create_time,
        T1.product_update_time
    from final_data T1
    INNER join full_index_data on T1.f_sku_id = full_index_data.f_sku_id
```

---

**文档生成**: 2026-01-30 13:41:29
