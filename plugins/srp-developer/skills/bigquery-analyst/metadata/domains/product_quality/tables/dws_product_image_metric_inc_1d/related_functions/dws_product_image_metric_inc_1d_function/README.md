# dws_product_image_metric_inc_1d_function

**函数全名**: `srpproduct-dc37e.favie_dw.dws_product_image_metric_inc_1d_function`
...
WITH product_image as (
        select 
            ...
            json_value(f_image.size_type) as image_size_type,
        from `favie_dw.dwd_favie_product_detail_inc_1d` ,unnest(json_query_array(PARSE_JSON(f_image_list))) as f_image
        where date(dt) = dt_param
        and f_image.f_link is not null
        and date(TIMESTAMP_SECONDS(cast(f_creates_at as INT64))) = dt_param
    ),
    ...
    product_metric as (
        select 
            ...
            site_parser_type,
            site_country_region,
            sum(inc_sku_uniq_cnt) as sku_uniq_cnt,
            sum(inc_spu_uniq_cnt) as spu_uniq_cnt
        from favie_dw.dws_favie_product_data_cube_inc_1d 
        where dt = dt_param
            and inc_sku_uniq_cnt > 0
        group by 
            dt,
            product_site,
            site_tier,
