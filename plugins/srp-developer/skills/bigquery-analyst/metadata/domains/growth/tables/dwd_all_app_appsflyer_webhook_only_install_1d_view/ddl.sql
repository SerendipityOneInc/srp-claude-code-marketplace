CREATE VIEW `srpproduct-dc37e.favie_dw.dwd_all_app_appsflyer_webhook_only_install_1d_view`
...
select 
	       ...
	       ,coalesce(if(ad_id='',null,ad_id),upper(source)) as ad_id
	       ,coalesce(if(ad_name='',null,ad_name),upper(source)) as ad_name
	       ,country_code
	       ,appsflyer_id
		   ,user_id
		   ,event_time
from (
	...
	(
		SELECT  dt
			   ...
			   ,user_id
			   ,event_time
		FROM `favie_dw.dwd_decofy_total_appsflyer_webhook_only_install_inc_1d_view`
		...
		SELECT  dt
			   ...
			   ,user_id
			   ,event_time
		FROM `favie_dw.dwd_gem_total_appsflyer_webhook_only_install_inc_1d_view`
         where appsflyer_id is not null 
	)
)
where rn=1
