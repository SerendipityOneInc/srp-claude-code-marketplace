...
**表全名**: `srpproduct-dc37e.favie_rpt.rpt_favie_gensmo_search_by_event_metric_inc_1d`
**层级**: RPT (报表层)
**业务域**: search
**表类型**: TABLE
**行数**: 0 行
...
| model_type | STRING | NULLABLE | 是否为Pro用户标识 |
...
SELECT
    dt,
    COUNT(*) as cnt
FROM `srpproduct-dc37e.favie_rpt.rpt_favie_gensmo_search_by_event_metric_inc_1d`
