CREATE TABLE `srpproduct-dc37e.favie_dw.dws_faive_gensmo_membership_points_metric_inc_1d`
(
  ...
  earn_points_user_cnt INT64 OPTIONS(description="获取过积分行为的用户标识"),
  earn_points_task_cnt INT64 OPTIONS(description="获取积分次数"),
  earn_points_points_amt INT64 OPTIONS(description="获取积分数量"),
  earn_points_not_checkin_user_cnt INT64 OPTIONS(description="通过主动完成任务获取积分行为的用户标识（不含被动checkin）"),
  ...
  earn_points_transaction_user_cnt INT64 OPTIONS(description="通过充值获取积分行为的用户标识"),
  ...
  consume_points_user_cnt INT64 OPTIONS(description="消耗过积分行为的用户标识"),
  ...
  consume_points_ge_task_claimed_user_cnt INT64 OPTIONS(description="用户当日消耗积分大于等于完成daily任务可获取的积分上限（目前登录10分+3次分享3分共13分）的用户标识"),
  consume_points_ge_checkin_user_cnt INT64 OPTIONS(description="用户当日消耗积分大于等于每日登录可获取的积分（目前登录为10分）的用户标识"),
  hit_limit_user_cnt INT64 OPTIONS(description="撞线用户标识"),
  points_expired INT64 OPTIONS(description="当日到期扣除积分")
)
PARTITION BY dt
OPTIONS(
