...
**表全名**: `srpproduct-dc37e.favie_dw.dws_savyo_growth_ad_metrics_inc_1d`
**层级**: DWS (汇总层)
**业务域**: advertising
**表类型**: TABLE
**行数**: 56 行
...
Savyo广告指标增量表，包含广告花费、转化、用户行为等指标
...
| app_name | STRING | NULLABLE | 应用名称：Savyo |
...
| channel | STRING | NULLABLE | 渠道：ON_LINE |
| attribution_method | STRING | NULLABLE | 归因方法：BOTH |
...
SELECT
    dt,
    COUNT(*) as cnt
FROM `srpproduct-dc37e.favie_dw.dws_savyo_growth_ad_metrics_inc_1d`
