CREATE TABLE `srpproduct-dc37e.favie_dw.dwd_favie_gensmo_events_inc_1d`
(
  dt DATE,
  session_id STRING,
  user_id STRING,
  ...
  event_items ARRAY<STRUCT<item_id STRING, item_name STRING, item_type STRING, item_index INT64>>,
  ...
  cal_pre_refer_event_items ARRAY<STRUCT<item_id STRING OPTIONS(description="参数ID"), item_name STRING OPTIONS(description="参数名称"), item_type STRING OPTIONS(description="参数类型"), item_index INT64 OPTIONS(description="参数在列表中的位置索引")>> OPTIONS(description="计算得出的前一个来源页面事件相关参数信息"),
  cal_refer_last_event_items ARRAY<STRUCT<item_id STRING OPTIONS(description="参数ID"), item_name STRING OPTIONS(description="参数名称"), item_type STRING OPTIONS(description="参数类型"), item_index INT64 OPTIONS(description="参数在列表中的位置索引")>> OPTIONS(description="计算得出的来源页面最后一个事件相关参数信息")
)
PARTITION BY dt
CLUSTER BY event_name
OPTIONS(
  require_partition_filter=true,
  labels=[("dataplex-dp-published-project", "srpproduct-dc37e"), ("dataplex-dp-published-location", "us-east1"), ("dataplex-dp-published-scan", "a88538379-0ef4-4fe3-abc7-3a0040d5b1b6")]
);