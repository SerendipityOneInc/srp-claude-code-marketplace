CREATE VIEW `srpproduct-dc37e.favie_dw.dwd_growth_ad_google_fivetran_by_ad_id_inc_1d_view`
AS SELECT 
       dt
...
select 
       ...
       when lower(e.account_name) like '%decofy%' then 'Decofy'  when lower(e.account_name) like '%savyo%' then 'Savyo'  
       else 'UNKNOWN' end as app_name
       ,case when lower(d.campaign_name) like '%ios%' then 'IOS'
       when lower(d.campaign_name) like '%android%' then 'Android' when lower(d.campaign_name) like '%web%' then 'WEB'  
...
SELECT  customer_id
       ...
       ,cost_micros/1000000          AS spend
       ,_fivetran_synced
       ,'GOOGLE_03' as  channel
FROM appsflyer_google03.ad_stats ) a
...
(
	SELECT  ad_group_id
	       ...
	       ,url_collections
	       ,ROW_NUMBER() OVER (PARTITION BY id ORDER BY  updated_at DESC ) AS rk
	FROM srpproduct-dc37e.appsflyer_google03.ad_history
)t6
WHERE rk = 1
) b
...
(
	SELECT  id
	       ...
	       ,tracking_url_template
	       ,ROW_NUMBER() OVER (PARTITION BY id ORDER BY updated_at DESC) AS rk
	FROM srpproduct-dc37e.appsflyer_google03.ad_group_history
)t7
WHERE rk = 1
) c
...
(
	SELECT  id
	       ...
	       ,video_brand_safety_suitability
	       ,ROW_NUMBER() OVER (PARTITION BY id ORDER BY updated_at DESC) AS rk
	FROM srpproduct-dc37e.appsflyer_google03.campaign_history
)t8
WHERE rk = 1
) d
...
(
	SELECT  id
	       ...
	       ,tracking_url_template
	       ,ROW_NUMBER() OVER (PARTITION BY id ORDER BY updated_at DESC) AS rk
	FROM srpproduct-dc37e.appsflyer_google03.account_history
)t9
WHERE rk = 1
) e
...
(
	SELECT  campaign_id
	       ...
	       ,status
	       ,ROW_NUMBER() OVER (PARTITION BY campaign_id, geo_target_constant_id ORDER BY updated_at DESC) AS rk
	FROM srpproduct-dc37e.appsflyer_google03.campaign_criterion_history
	WHERE geo_target_constant_id IS NOT NULL
	  AND type = 'LOCATION'
	  AND negative = false
)t10
...
left join (
       ...
       ,target_type as geo_target_type
       ,status as geo_target_status
       FROM srpproduct-dc37e.appsflyer_google03.geo_target
) geo_info
...
SELECT  customer_id
       ...
       ,cost_micros/1000000          AS spend
       ,_fivetran_synced
       ,'GOOGLE_04' as  channel
FROM google_inhouse_ads.ad_stats ) a
...
(
	SELECT  ad_group_id
	       ...
	       ,url_collections
	       ,ROW_NUMBER() OVER (PARTITION BY id ORDER BY  updated_at DESC ) AS rk
	FROM srpproduct-dc37e.google_inhouse_ads.ad_history
)t6
WHERE rk = 1
) b
...
(
	SELECT  id
	       ...
	       ,tracking_url_template
	       ,ROW_NUMBER() OVER (PARTITION BY id ORDER BY updated_at DESC) AS rk
	FROM srpproduct-dc37e.google_inhouse_ads.ad_group_history
)t7
WHERE rk = 1
) c
...
(
	SELECT  id
	       ...
	       ,video_brand_safety_suitability
	       ,ROW_NUMBER() OVER (PARTITION BY id ORDER BY updated_at DESC) AS rk
	FROM srpproduct-dc37e.google_inhouse_ads.campaign_history
)t8
WHERE rk = 1
) d
...
(
	SELECT  id
	       ...
	       ,tracking_url_template
	       ,ROW_NUMBER() OVER (PARTITION BY id ORDER BY updated_at DESC) AS rk
	FROM srpproduct-dc37e.google_inhouse_ads.account_history
)t9
WHERE rk = 1
) e
...
(
	SELECT  campaign_id
	       ...
	       ,status
	       ,ROW_NUMBER() OVER (PARTITION BY campaign_id, geo_target_constant_id ORDER BY updated_at DESC) AS rk
	FROM srpproduct-dc37e.google_inhouse_ads.campaign_criterion_history
	WHERE geo_target_constant_id IS NOT NULL
	  AND type = 'LOCATION'
	  AND negative = false
)t10
...
left join (
       ...
       ,target_type as geo_target_type
       ,status as geo_target_status
       FROM srpproduct-dc37e.google_inhouse_ads.geo_target
) geo_info
...
SELECT  customer_id
       ...
       ,cost_micros/1000000          AS spend
       ,_fivetran_synced
       ,'GOOGLE_05' as  channel
FROM google_yla_03.ad_stats ) a
...
(
	SELECT  ad_group_id
	       ...
	       ,url_collections
	       ,ROW_NUMBER() OVER (PARTITION BY id ORDER BY  updated_at DESC ) AS rk
	FROM srpproduct-dc37e.google_yla_03.ad_history
)t6
WHERE rk = 1
) b
...
(
	SELECT  id
	       ...
	       ,tracking_url_template
	       ,ROW_NUMBER() OVER (PARTITION BY id ORDER BY updated_at DESC) AS rk
	FROM srpproduct-dc37e.google_yla_03.ad_group_history
)t7
WHERE rk = 1
) c
...
(
	SELECT  id
	       ...
	       ,video_brand_safety_suitability
	       ,ROW_NUMBER() OVER (PARTITION BY id ORDER BY updated_at DESC) AS rk
	FROM srpproduct-dc37e.google_yla_03.campaign_history
)t8
WHERE rk = 1
) d
...
(
	SELECT  id
	       ...
	       ,tracking_url_template
	       ,ROW_NUMBER() OVER (PARTITION BY id ORDER BY updated_at DESC) AS rk
	FROM srpproduct-dc37e.google_yla_03.account_history
)t9
WHERE rk = 1
) e
...
(
	SELECT  campaign_id
	       ...
	       ,status
	       ,ROW_NUMBER() OVER (PARTITION BY campaign_id, geo_target_constant_id ORDER BY updated_at DESC) AS rk
	FROM srpproduct-dc37e.google_yla_03.campaign_criterion_history
	WHERE geo_target_constant_id IS NOT NULL
	  AND type = 'LOCATION'
	  AND negative = false
)t10
...
left join (
       ...
       ,target_type as geo_target_type
       ,status as geo_target_status
       FROM srpproduct-dc37e.google_yla_03.geo_target
) geo_info
...
select 
       ...
       ,case when lower(d.campaign_name) like '%ios%' then 'IOS'
       when lower(d.campaign_name) like '%android%' then 'Android' when lower(d.campaign_name) like '%web%' then 'WEB'
       when lower(e.account_name) like '%android%' then 'Android'
       when lower(e.account_name) like '%ios%' then 'IOS'
       when lower(e.account_name) like '%web%' then 'WEB'
       else 'UNKNOWN' end as platform
from (
SELECT  customer_id
       ,date                         AS dt
       ...
       ,cost_micros/1000000          AS spend
       ,_fivetran_synced
       ,'GOOGLE_06_SAVYO' as  channel
FROM google_ads_savyo.ad_stats ) a
...
(
	SELECT  ad_group_id
	       ...
	       ,url_collections
	       ,ROW_NUMBER() OVER (PARTITION BY id ORDER BY  updated_at DESC ) AS rk
	FROM srpproduct-dc37e.google_ads_savyo.ad_history
)t6
WHERE rk = 1
) b
...
(
	SELECT  id
	       ...
	       ,tracking_url_template
	       ,ROW_NUMBER() OVER (PARTITION BY id ORDER BY updated_at DESC) AS rk
	FROM srpproduct-dc37e.google_ads_savyo.ad_group_history
)t7
WHERE rk = 1
) c
...
(
	SELECT  id
	       ...
	       ,video_brand_safety_suitability
	       ,ROW_NUMBER() OVER (PARTITION BY id ORDER BY updated_at DESC) AS rk
	FROM srpproduct-dc37e.google_ads_savyo.campaign_history
)t8
WHERE rk = 1
) d
...
(
	SELECT  id
	       ...
	       ,tracking_url_template
	       ,ROW_NUMBER() OVER (PARTITION BY id ORDER BY updated_at DESC) AS rk
	FROM srpproduct-dc37e.google_ads_savyo.account_history
)t9
WHERE rk = 1
) e
...
(
	SELECT  campaign_id
	       ...
	       ,status
	       ,ROW_NUMBER() OVER (PARTITION BY campaign_id, geo_target_constant_id ORDER BY updated_at DESC) AS rk
	FROM srpproduct-dc37e.google_ads_savyo.campaign_criterion_history
	WHERE geo_target_constant_id IS NOT NULL
	  AND type = 'LOCATION'
	  AND negative = false
)t10
...
left join (
       ...
       ,target_type as geo_target_type
       ,status as geo_target_status
       FROM srpproduct-dc37e.google_ads_savyo.geo_target
) geo_info
