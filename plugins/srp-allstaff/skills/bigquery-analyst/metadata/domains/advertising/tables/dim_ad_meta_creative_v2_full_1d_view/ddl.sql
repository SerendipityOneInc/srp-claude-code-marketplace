CREATE VIEW `srpproduct-dc37e.favie_dw.dim_ad_meta_creative_v2_full_1d_view`
...
( --可以关联 上
	SELECT  id                AS video_id
	       ,MAX(source)       AS video_url
	       ,MAX(title)        AS video_name
	       ,MAX(updated_time) AS video_updated_at
	       ,MIN(created_time) AS video_created_at
	FROM `srpproduct-dc37e.fivetran_facebook_ads_full.ad_video_history`
	WHERE id is not null
	GROUP BY  id
) b
...
SELECT  source
       ...
       ,CASE WHEN internal_creative_url is not null AND lower(creative_type) = 'video' THEN 'INTENAL_VIDEO_LIBRARY'
             ...
             WHEN external_website_creative_url is not null THEN 'KOL_VIDEO'  ELSE 'UNKNOWN' END AS creative_source
FROM
(
	SELECT  z1.source
	       ,z1.platform
	       ...
	       ,z1.image_created_at
	       ,z1.image_updated_at
		   ,REGEXP_EXTRACT(external_website_creative_url, r'/([^/?#]+)/?$') as kol_id
	...
	(
		...
		WHERE dt = (
		SELECT  MAX(dt)
		FROM `favie_dw.dim_ad_all_app_creative_full_1d`) AND source = 'Meta'
