CREATE VIEW `srpproduct-dc37e.favie_dw.dim_ad_tiktok_creative_multiple_full_1d_view`
...
SELECT  source
       ...
       ,coalesce(if(a.creative_id='UNKNOWN',null,a.creative_id),cast(e.material_id as string)) as f_creative_id
...
(
	...
	--material id是视频的唯一标识,也就是说creative id是 素材的唯一id 
	SELECT  *
	FROM
	(
...
(
	...
	(
		SELECT  external_creative_id
		    ...
		    when lower(creative_type)='image' and internal_creative_id is not null  then 'jpg' else format end ) as format 
