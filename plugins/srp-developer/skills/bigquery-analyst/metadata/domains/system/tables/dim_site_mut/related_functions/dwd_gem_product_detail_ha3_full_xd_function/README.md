# dwd_gem_product_detail_ha3_full_xd_function

**函数全名**: `srpproduct-dc37e.favie_dw.dwd_gem_product_detail_ha3_full_xd_function`
...
with source_data as (
    ...
    final_data as (
        ...
        SELECT
            ...
            0 AS recent_sales, -- 从 `extended_info` 直接解析, 这个字段目前是一个字符串，类似1000+,所以不能直接转，先默认设置为0
            CASE 
            ...
            COALESCE(T11.seo_title, T10.enhanced_title) as seo_title,
            COALESCE(T11.seo_text_info, T10.enhanced_title) as seo_text_info,
            T8.image_score,
