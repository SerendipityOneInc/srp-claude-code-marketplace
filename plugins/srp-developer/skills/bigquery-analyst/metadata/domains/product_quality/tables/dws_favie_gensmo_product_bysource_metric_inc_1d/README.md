...
**表全名**: `srpproduct-dc37e.favie_dw.dws_favie_gensmo_product_bysource_metric_inc_1d`
**层级**: DWS (汇总层)
**业务域**: product_quality
**表类型**: TABLE
**行数**: 481,658,277 行
...
| cal_pre_refer | STRING | NULLABLE | 前一跳来源页面 |
...
| product_external_jump_click_device_id | STRING | NULLABLE | 商品外部跳转事件设备ID |
| product_detail_pv_cnt | INTEGER | NULLABLE | 商品详情卡片浏览次数 |
| product_detail_pv_device_id | STRING | NULLABLE | 商品详情卡片浏览设备ID |
...
SELECT
    dt,
    COUNT(*) as cnt
FROM `srpproduct-dc37e.favie_dw.dws_favie_gensmo_product_bysource_metric_inc_1d`
