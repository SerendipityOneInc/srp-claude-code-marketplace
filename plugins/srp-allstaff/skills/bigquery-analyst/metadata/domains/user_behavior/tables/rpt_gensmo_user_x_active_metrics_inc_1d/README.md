...
**表全名**: `srpproduct-dc37e.favie_rpt.rpt_gensmo_user_x_active_metrics_inc_1d`
**层级**: RPT (报表层)
**业务域**: user_behavior
**表类型**: TABLE
**行数**: 81,217,597 行
...
| active_period | INTEGER | NULLABLE | 活跃周期：1、7、30天 |
| active_user_cnt | INTEGER | NULLABLE | 1日活跃用户数 |
| user_duration | FLOAT | NULLABLE | 1日活跃用户总停留时长 |
...
SELECT
    dt,
    COUNT(*) as cnt
FROM `srpproduct-dc37e.favie_rpt.rpt_gensmo_user_x_active_metrics_inc_1d`
