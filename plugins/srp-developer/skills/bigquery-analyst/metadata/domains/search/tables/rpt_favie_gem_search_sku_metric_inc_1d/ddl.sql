CREATE TABLE `srpproduct-dc37e.favie_dw.rpt_favie_gem_search_sku_metric_inc_1d`
(
  ...
  site_domain STRING OPTIONS(description="站点完整域名或域标识"),
  site_top_domain STRING OPTIONS(description="站点顶级域/平台（例如：某大型平台的主域）"),
  site_tier STRING OPTIONS(description="站点层级/分层标识（例如：一级/二级/三级站点分类）"),
  site_type STRING OPTIONS(description="站点类型（例如：电商/内容/品牌站等）"),
  site_categories STRING OPTIONS(description="站点所属品类或分类路径，逗号或分隔符分隔"),
  site_parser_type STRING OPTIONS(description="用于解析该站点的解析器类型或版本"),
  site_country_region STRING OPTIONS(description="站点所属国家或地区"),
  ...
  site_rank STRING OPTIONS(description="站点排名或权重指标")
)
PARTITION BY dt;