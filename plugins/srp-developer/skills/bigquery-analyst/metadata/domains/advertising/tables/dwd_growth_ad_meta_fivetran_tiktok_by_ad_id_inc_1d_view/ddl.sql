CREATE VIEW `srpproduct-dc37e.favie_dw.dwd_growth_ad_meta_fivetran_tiktok_by_ad_id_inc_1d_view`
AS SELECT  a.stat_time_day                                                                                               AS dt
       ...
       ,CASE WHEN LOWER(e.name) LIKE '%gensmo%' THEN 'Gensmo'
             WHEN LOWER(e.name) LIKE '%pickly%' THEN 'Pickly'
             WHEN LOWER(e.name) LIKE '%decofy%' THEN 'Decofy'  ELSE 'UNKNOWN' END                                     AS app_name
